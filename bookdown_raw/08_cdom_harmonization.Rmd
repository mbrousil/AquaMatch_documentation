...
params:
  ...
  cdom_chars: NA
  tiering_record: NA
  param_drop_record: NA
  cdom_unit_table: NA
  cdom_misc_flag_table: NA
...
documented_drops <- params$documented_drops %>%
  filter(grepl(pattern = "cdom", x = step, ignore.case = TRUE),
         !((step == "cdom harmonization") & short_reason == "Start"))
cdom_chars <- params$cdom_chars
tiering_record <- params$tiering_record
param_drop_record <- params$param_drop_record
cdom_unit_table <- params$cdom_unit_table
cdom_misc_flag_table <- params$cdom_misc_flag_table
```
...
At the start of the pre-harmonization process the raw CDOM WQP dataset contains `r step_1_rows` rows.
...
Once ready for harmonization, the CDOM WQP dataset contains the following user-defined `CharacteristicNames`: `r cdom_chars`.

<img src="in/cdom/cdom_stacked_char_names.png" alt="Stacked bar chart with counts of CharacteristicName records" width="650"/>

These `CharacteristicNames` are chosen in order to select for only those measurements that pertain to CDOM. We selected them by reviewing options in the
`Characteristics` field of the [Water Quality Portal Filter Results](https://www.waterqualitydata.us/#advanced=true) page.
...
Records that do not meet the criteria of one of the `parameter` entries above are dropped (n = `r comma(sum(param_drop_record$n))`). A summary of the types of dropped records and their row counts is below:

```{r fig.alt="Table containing combinations of CDOM record summary information that were dropped while categorizing by parameter."}
...
```{r fig.alt="Table containing combinations of CDOM record summary information and corresponding tiers."}
...
No records should be removed by this process and so there are `r step_13_dropped` rows dropped leaving `r step_13_rows` remaining in the harmonized CDOM dataset.
...
No records should be removed by this process and so there are `r step_14_dropped` rows dropped leaving `r step_14_rows` remaining in the harmonized CDOM dataset.
...
`r step_16_dropped` rows dropped leaving `r step_16_rows` remaining in the final harmonized and aggregated CDOM dataset.
